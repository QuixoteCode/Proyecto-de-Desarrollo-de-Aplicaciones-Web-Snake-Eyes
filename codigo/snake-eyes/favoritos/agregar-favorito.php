    ...
    <section id="contenido">
        <section class="p-4">
            <?php
                ...
                //Comprobamos que se ha pasado la id del usuario y la id del producto
                if (isset($_SESSION['user_id']) && isset($_POST['producto_id'])) {
                    $user_id = $_SESSION['user_id'];
                    $producto_id = $_POST['producto_id'];
    
                    //Comprobamos si el producto ya está presente en favoritos
                    $sql = "SELECT * FROM favoritos WHERE usuario_id = ? AND producto_id = ?";
                    $statement = $conexion->prepare($sql);
                    $statement->bind_param("ii", $user_id, $producto_id);
                    $statement->execute();
                    $resultado = $statement->get_result();
    
                    //En caso de que no esté presente agregamos el producto a favoritos
                    if ($resultado->num_rows == 0) {
                        $insert_sql = "INSERT INTO favoritos (usuario_id, producto_id) VALUES (?, ?)";
                        $insert_statement = $conexion->prepare($insert_sql);
                        $insert_statement->bind_param("ii", $user_id, $producto_id);
                        $insert_statement->execute();
    
                        if ($insert_statement->affected_rows > 0) {
                            echo "<p>Producto añadido a favoritos.</p>";
                        } else {
                            echo "<p>Error al añadir a favoritos.</p>";
                        }

                        $insert_statement->close();
                    } else {
                        echo "<p>El producto ya está en favoritos.</p>";
                    }
                    ...
                    if (!isset($_SESSION['user_id'])) {
                        echo "<p>Por favor, <a href='../dashboard/dashboard.php'>inicia sesión</a> para añadir este producto a favoritos.</p>";
                    }else{
