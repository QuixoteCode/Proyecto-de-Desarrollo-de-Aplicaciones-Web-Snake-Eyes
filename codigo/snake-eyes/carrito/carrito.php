...
<body class="bg-dark text-white">
    <nav class="navbar navbar-expand-lg bg-danger navbar-light fixed-top">
        ...
        <section id="contenido">
            <section class="p-4">
                <?php
                    //Verificamos si se ha enviado el formulario vaciar el carrito
                    if(isset($_POST['vaciar_carrito'])) {
                        //Limpia el carrito de contenido reemplazando el contenido de $_SESSION['carrito'] con un array vacío, unset es una alternativa a esta forma de hacerlo
                        $_SESSION['carrito'] = [];
                        echo "El carrito ha sido vaciado. ";
                    }

                    //Verificamos si se ha enviado el formulario para eliminar un elemento del carrito
                    if (isset($_POST['eliminar_elemento'])) {
                        $indice = $_POST['indice'];
                        if (isset($_SESSION['carrito'][$indice])) {
                            unset($_SESSION['carrito'][$indice]);
                            echo "<p>El producto ha sido eliminado del carrito.</p><br>";
                        }
                    }
        
                    //Verificamos si hay datos contenidos en el carrito
                    if(isset($_SESSION['carrito']) && !empty($_SESSION['carrito'])) {
                        ...
                        foreach ($_SESSION['carrito'] as $indice => $objeto) {
                            echo '<tr>
                                ...
                                <td>' . htmlspecialchars($objeto['cantidad']) . '</td>
                                <td>' . htmlspecialchars($objeto['precio']) . '</td>
                                <td>';
                                //Botón para eliminar un único elemento del carrito
                                    echo '<form method="post" class="d-inline-block">
                                        <input type="hidden" name="indice" value="' . $indice . '">
                                        <button type="submit" name="eliminar_elemento" class="btn btn-danger">Eliminar</button>
                                    </form>
                                </td>
                            </tr>';
                        }
                        ...
                        //Botón que nos permite vaciar el carrito
                        echo '<form method="post" class="d-inline-block">';
                        echo '<button type="submit" name="vaciar_carrito" class="btn btn-danger">Vaciar Carrito</button>';
                        echo '</form>';

                        if(isset($_SESSION['user_id'])){
                            echo '<a href="pago.php" class="btn btn-danger d-inline-block ml-2">Proceder al pago</a>';
                        }
                    } else {
