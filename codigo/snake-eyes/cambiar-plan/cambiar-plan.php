<?php
    ...
    //Verificamos si el formulario fue enviado en el documento
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        //Obtenemos los datos que han sido pasados
        $plan_nuevo = $_POST['plan_nuevo'];
        $user_id = $_SESSION['user_id']; 

        //Preparamos la consulta para actualizar el plan del usuario en la base de datos
        $sql = "UPDATE usuarios SET plan = ? WHERE id = ?";
        $statement = $conexion->prepare($sql);
        //Usamos "s" debido a que $plan_nuevo es de tipo cadena ("string") e "i" debido a que $user_id es de tipo entero ("integer")
        $statement->bind_param("si", $plan_nuevo, $user_id);

        //Ejecutamos la consulta
        if ($statement->execute()) {
            echo "<p class='m-2 p-2'>Plan cambiado con éxito.</p>";
        } else {
            echo "<p class='m-2 p-2'>Error al cambiar el plan.</p>";
        }
...
<head>
    ...
    <link rel="stylesheet" href="cambiar-plan.css">
</head>
<body class="bg-dark text-light">
                ...
                ?>
                    <li class="nav-item dropdown p-4">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="nav-notificacion">
                            <li><a class="dropdown-item" href="cambiar-plan.php"><img src="../imagenes/notificacion.png" alt="Oferta" height="333"></a></li>
                        </ul>
                    </li>
                <?php
                    }
    ...
    <section id="contenido">
        <section class="col-12 col-sm-6">
            <?php if(isset($_SESSION['user_id'])): ?>
                <p>Cambia ahora tu plan a Oro y recibe...</p>
                <ul>
                    <li>3 meses gratis.</li>
                    <li>Mejoras cosméticas en el foro.</li>
                    <li>Envíos gratuitos.</li>
                    <li>Soporte preferencial a tu email.</li>
                    <li>Conocer las actualizaciones y los nuevos lanzamientos antes que nadie.</li>
                </ul>
                <p>¡Y mucho más!</p>
                ...
                <p>Selecciona un nuevo plan</p>
                    <select name="plan_nuevo" id="plan_nuevo" class="form-control" required>
                    ...
                    <button type="submit" class="btn btn-danger btn-lg m-5 p-3">Cambiar plan</button>
                </form>
            <?php else: ?>
                <p class="m-2 p-2">Por favor, <a href="../dashboard/dashboard.php">inicia sesión</a> para modificar tu plan</p>
            <?php endif; ?>
        </section>
    </section>
    ...
    <script src="cambiar-plan.js"></script>
</body>
</html>