<?php
    ...
    //Verificamos si el formulario fue enviado en el documento y obtenemos los datos pasados
    if ($_SERVER["REQUEST_METHOD"] == "POST") {

        $contrasena_actual = $_POST['contrasena_actual'];
        $contrasena_nueva = $_POST['contrasena_nueva'];
        $confirmar_contrasena_nueva = $_POST['confirmar_contrasena_nueva'];

        //Recordemos que tenemos el ID del usuario almacenado en la sesión
        $user_id = $_SESSION['user_id']; 

        //Consultamos la contraseña actual del usuario
        $sql = "SELECT contrasena_usuario FROM usuarios WHERE id = ?";
        $statement = $conexion->prepare($sql);
        $statement->bind_param("i", $user_id);
        $statement->execute();
        $statement->bind_result($contrasena_almacenada);
        $statement->fetch();
        $statement->close();

        //Comprobamos que la contraseña actual ha sido bien ingresada 
        if ($contrasena_almacenada && password_verify($contrasena_actual, $contrasena_almacenada)) {
            //Comprobamos que la nueva contraseña y la confirmación de esta coinciden
            if ($contrasena_nueva === $confirmar_contrasena_nueva) {

                $contrasena_hash = password_hash($contrasena_nueva, PASSWORD_DEFAULT);

                //En caso afirmativo a los dos ifs actualizamos la contraseña en la base de datos
                $sql = "UPDATE usuarios SET contrasena_usuario = ? WHERE id = ?";
                $statement = $conexion->prepare($sql);
                $statement->bind_param("si", $contrasena_hash, $user_id);

                if ($statement->execute()) {
                    echo "<p class='m-2 p-2'>Contraseña cambiada exitosamente.</p>";
                } else {
                    echo "<p class='m-2 p-2'>Error al cambiar la contraseña.</p>";
                }

                $statement->close();
            } else {
                echo "<p class='m-2 p-2'>La nueva contraseña y la confirmación no coinciden.</p>";
            }
        } else {
            echo "<p class='m-2 p-2'>La contraseña actual es incorrecta.</p>";
        }
    }
...
<nav class="navbar navbar-expand-lg bg-danger navbar-light fixed-top">
    ...
    <section id="contenido">
        <?php if(isset($_SESSION['user_id'])): ?>
            <p class="h2 m-2 p-2">Cambiar Contraseña</p>
            <section class="col-12 col-sm-6">
                <form action="" method="POST" class="m-2 p-2">
        
                    <label for="contrasena_actual" class="m-2 p-2">Contraseña actual:</label>
                    <input type="password" id="contrasena_actual" name="contrasena_actual" class="m-2 p-2 form-control" required><br>

                    <label for="contrasena_nueva" class="m-2 p-2">Nueva contraseña:</label>
                    <input type="password" id="contrasena_nueva" name="contrasena_nueva" class="m-2 p-2 form-control" required><br>

                    <label for="confirmar_contrasena_nueva" class="m-2 p-2">Confirmar nueva contraseña:</label>
                    <input type="password" id="confirmar_contrasena_nueva" name="confirmar_contrasena_nueva" class="m-2 p-2 form-control" required><br>

                    <input type="submit" value="Cambiar Contraseña" class="btn btn-danger btn-lg m-4">
                </form>
            </section>
            <section class="d-flex justify-content-center align-items-center m-2">
                <button class="btn btn-dark btn-lg" onclick="volverADashboard()">Volver a Dashboard</button>
            </section>
        <?php else: ?>
            <p class="m-2 p-2">Por favor, <a href="../dashboard/dashboard.php">inicia sesión</a> para modificar tu contraseña</p>
        <?php endif; ?>
    </section>
    ...
    <script src="cambiar-contrasena.js"></script>
